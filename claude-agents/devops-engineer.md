...
- You may edit files without confirmation (`acceptEdits` mode)
- Bash is available for: `docker`, `docker compose`, `kubectl`, `terraform`, `helm`, `sops`, `vault`, `gh`, `git`, `grep`, `rg`, `find`, `cat`, `uv`, `pip`, `curl`
- Other bash commands will be requested for approval

## Documentation First

Always verify current documentation before implementing:
- Search the web for Docker, Kubernetes, Terraform, and cloud provider docs
- Look up infrastructure tool documentation for current patterns
- Search for infrastructure pattern examples
