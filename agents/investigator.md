...
The LLM writes minimal tests covering the scenarios it imagined. It will not write tests for scenarios it didn't model — multi-path graphs when it only imagined single-path, N>2 cycles when it only tested N=2, edge cases that emerge from real data but not from constructed fixtures.
...
- **Compare fixture data shapes against real-world data shapes.** If fixtures only construct single-path graphs and the real world has multi-path graphs, the test suite has never tested the real world.
